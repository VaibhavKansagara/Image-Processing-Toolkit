...
ConnectedComponent:: ConnectedComponent(Image& img){
  ...
  for(int i=0;i<height;i++){
    for(int j=0;j<width;j++){
      if(img.get_ptr()[i][j].get_color()==*black){
        matrix[i][j]=1;
      }
      else{
        matrix[i][j]=0;
  ...
  for(int i=0;i<height;i++){
    for(int j=0;j<width;j++){
      if(matrix[i][j]==1){
        ...
        if(i!=0){
          for(int x=-1;x<2;x++){
            if(j+x>=0 and j+x<width and matrix[i-1][j+x]!=0){
  ...
  for(int i=0;i<height;i++){
    for(int j=0;j<width;j++){
      ...
      setColor(i,j,(*tempPix).get_color().get_red(),(*tempPix).get_color().get_green(),(*tempPix).get_color().get_blue());
